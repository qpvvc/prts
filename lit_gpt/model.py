...
class GPT(nn.Module):
    ...
    def extract(self, a, t, x_shape):
        ...
        return out.reshape(*((1,) * (len(x_shape) - 1))).to(t.device)

    def apply_noisy(self, module: nn.Module, step, total_step, n_layer):
        t = torch.tensor([step])
        betas = self.linear_beta_schedule(timesteps=total_step)
        ...
        if isinstance(module, nn.Embedding):
            noisy = torch.randn(module.weight.size()) * math.sqrt(2.0 / 5 / module.weight.size(1))
            sqrt_alphas_cumprod_t = self.extract(sqrt_alphas_cumprod, t, module.weight.shape)
            sqrt_one_minus_alphas_cumprod_t = self.extract(
                sqrt_one_minus_alphas_cumprod, t, module.weight.shape
            )
            module.weight.data = sqrt_alphas_cumprod_t * module.weight.data + sqrt_one_minus_alphas_cumprod_t * noisy
        elif isinstance(module, nn.Linear):
            # fan-in variance scaling intializer
            noisy = torch.randn(module.weight.size()) * math.sqrt(2.0 / 5 / module.weight.size(1))
            sqrt_alphas_cumprod_t = self.extract(sqrt_alphas_cumprod, t, module.weight.shape)
        ...
        for name, p in module.named_parameters():
            if (name == "proj.weight" and isinstance(module, LLaMAMLP)) or (name == "w3.weight" and isinstance(module, SwiGLU)):  #if use xformer swiglu, fc2 layer will be renamed to w3
                noisy = torch.randn(p.size()) * (1 / math.sqrt(p.shape[-1]) / n_layer)
                sqrt_alphas_cumprod_t = self.extract(sqrt_alphas_cumprod, t, p.shape)
                sqrt_one_minus_alphas_cumprod_t = self.extract(
                    sqrt_one_minus_alphas_cumprod, t, p.shape
                )
                p.data = sqrt_alphas_cumprod_t * p.data + sqrt_one_minus_alphas_cumprod_t * noisy

    def edit_float(self, input_tensor, n):
        shape = input_tensor.shape
        pow = torch.full(shape, n)
        powed_tensor = input_tensor * torch.pow(10, pow)
        integer_part = torch.floor(powed_tensor)
        decimal_part = ((powed_tensor - integer_part) * 10 - torch.floor((powed_tensor - integer_part) * 10))
        noise = torch.randint(0, 10, size=shape)
        return (integer_part + (decimal_part + noise) / 10) / torch.pow(10, pow)

    def apply_random(self, module: nn.Module, n):
        if isinstance(module, nn.Embedding):
            module.weight.data = self.edit_float(module.weight.data, n)
        elif isinstance(module, nn.Linear):
            # fan-in variance scaling intializer
            module.weight.data = self.edit_float(module.weight.data, n)
            if module.bias is not None:
        ...
        for name, p in module.named_parameters():
            if (name == "proj.weight" and isinstance(module, LLaMAMLP)) or (name == "w3.weight" and isinstance(module, SwiGLU)):  #if use xformer swiglu, fc2 layer will be renamed to w3
                p.data = self.edit_float(p.data, n)
